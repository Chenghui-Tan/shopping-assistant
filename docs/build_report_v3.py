...
heading(1, "Abstract")
para(
    "Online shopping platforms have reduced search cost but purchasing decisions remain cognitively demanding. "
    "Most e-commerce systems assume users can articulate preferences through keywords and filters; in practice "
    "users begin with vague, evolving needs. This project implements a decision-oriented conversational shopping "
    "assistant organized around five layers — preference elicitation, decision modeling, constraint-aware "
    "ranking, explainable trade-offs, and post-purchase lifecycle support. Its central architectural choice is "
    "a trust boundary: Claude (Anthropic) handles natural-language work, while the ranking itself is carried "
    "out by deterministic rule-based code and never invokes the language model. This split yields auditability, "
    "stability across model updates, and explainability by construction. A Playwright ETL pipeline collects "
    "and normalizes 100 products across three categories from Amazon and Target; a React + FastAPI stack "
    "completes the implementation. Scenario-based evaluation shows the system addresses structural gaps — "
    "preference continuity, decision transparency, delivery information, lifecycle support — that are absent "
    "from Amazon Rufus, ChatGPT Shopping, Google Shopping, and Perplexity."
)
page_break()
...
para(
    ...
    "remain cognitively demanding. Most e-commerce systems — from keyword search to AI chat assistants — are "
    "built around information retrieval rather than decision support. They assume users can articulate needs "
    "as queries and optimize for engagement metrics such as click-through rate rather than decision "
    "confidence [1]."
)
para(
    "In practice, users begin with vague, evolving needs. A user searching for a 'convenient kitchen device' "
    "may not know whether they want a smart display, a multi-cooker, or an organizer. They balance competing "
    "priorities — price, delivery, brand, size, usability — without structured support. Existing tools fail "
    "to preserve preference continuity across interactions, surface decision-critical attributes such as "
    "delivery time, or explain why one product is ranked above another [2]."
)
para(
    "This project addresses that gap. The proposed system integrates five logical layers — preference "
    "elicitation, decision modeling, recommendation ranking, explainability, and lifecycle support — into a "
    "unified conversational workflow, backed by a real ETL pipeline collecting data from Amazon and Target "
    "and a full-stack implementation using React, FastAPI, and Claude (Anthropic, Opus 4.6) as the "
    "natural-language layer. Section 4.1 details the organizing architectural commitment of the system: a "
    "trust boundary that assigns language work to Claude and ranking to deterministic rule-based code."
)
page_break()
...
# ──────────────────────────────────────────────────────────────────────────────
# 4.1 — Architecture & Trust Boundary
# ──────────────────────────────────────────────────────────────────────────────
heading(2, "Architecture & Trust Boundary", "4.1")
para(
    "The organizing architectural decision is a deliberate split between probabilistic language work handled "
    "by Claude and deterministic ranking logic handled by rule-based code. Every downstream module sits on "
    "one side of this boundary. For a decision-support system, three properties are non-negotiable: a user "
    "must be able to audit why a product is ranked where it is; the developer must reproduce a ranking across "
    "runs; and ranking behavior must not drift as upstream language-model versions change. A pure LLM-wrapper "
    "architecture cannot deliver these reliably. Concretely, Claude is invoked only at four well-scoped "
    "call sites (Section 4.4.1); the recommendation engine is pure Python with no LLM calls (Section 4.4.2). "
    "Figure 1 illustrates the split."
)
...
# ── 4.1.1 Why this approach works ─────────────────────────────────────────────
heading(3, "Why This Approach Works", "4.1.1")
bullet("Auditability. Every ranking has a visible weighted sum — a reviewer asking \"why is this #1?\" gets pointed to numbers, not model interpretability output.")
bullet("Stability. When the Claude API version changes, the ranking does not. Only the language layer moves; the decision logic stays reproducible across model updates.")
bullet("Cheap iteration. Ranking changes are Python edits; LLM changes are prompt edits. Neither blocks the other.")
bullet("Explainability by construction. Score components are preserved alongside the product, so \"explain this ranking\" is a lookup, not a second AI inference.")
...
heading(2, "Ranking Models", "4.4")
para(
    "Two model components drive recommendation logic, one on each side of the trust boundary: Claude "
    "(Anthropic) on the language side and a deterministic two-tier weighted-scoring engine on the decision "
    "side."
)
...
heading(2, "Frontend & Backend", "4.5")
para(
...
para(
    "This project demonstrates that decision-oriented conversational shopping is technically feasible as a "
    "full-stack system built with currently available AI models. The five-layer architecture — preference "
    "elicitation, decision modeling, ranking, explainability, lifecycle support — integrates into a unified "
    "experience that addresses structural gaps in existing tools. Its organizing idea is the trust boundary: "
    "the language model handles what it is good at (parsing, explaining, suggesting); the deterministic "
    "engine handles what must be reliable (ranking). Evaluated against click-through rate, this approach is "
    "uninteresting. Evaluated against decision quality, transparency, and auditability — the properties that "
    "actually matter for high-consideration purchases — it closes gaps no surveyed baseline addresses."
)
para("Key contributions:")
bullet("Reframing e-commerce recommendation as a decision-support task, distinct from engagement-optimized retrieval.")
bullet("A trust-boundary architecture that yields auditability, reproducibility across model updates, and explainability by construction.")
bullet("A two-tier weighted scoring engine with dynamic general weights derived from user priorities, blended with category-specific feature weights via a tunable α.")
bullet("A Playwright ETL pipeline with normalization, two-phase dedup, and primary/fallback/supplement orchestration.")
...
para(
    "This capstone was completed individually by Chenghui Tan. All components — architecture, Playwright "
    "ETL pipeline, two-tier recommendation engine, Claude prompt design and response validation, React SPA "
    "and FastAPI backend, evaluation, and this report — were designed, implemented, and evaluated by the "
    "author."
)

heading(2, "AI Tool Contributions", "7.2")
para(
    "Claude (Anthropic, Opus 4.6) serves as the natural-language layer of the application (four call sites) "
    "and was used during development for code assistance and drafting. GitHub Copilot was used for code "
    "completion. All AI-generated code was reviewed, tested, and modified by the author. The system "
    "architecture, research framing, evaluation design, and written analysis are the author's own."
)
page_break()
...
# ── Save ──────────────────────────────────────────────────────────────────────
out = "/Users/sabrina/Projects/shopping-assistant-4/docs/capstone_report_v3.docx"
doc.save(out)
print(f"Saved → {out}")
