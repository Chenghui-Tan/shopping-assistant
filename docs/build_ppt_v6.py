...
add_text(s,
         "→  Our system is the only one that addresses all seven gaps simultaneously.  "
...
clusters = [
    ("🧠", "Understand You", TEAL,
     ["Real Frustration → Category", "Decision-Relevant Q&A",
      "Multi-Select Use Cases"],
     "ecosystem · placement · privacy · screen size"),
    ("👤", "Built for You", MID_BLUE,
     ["Preference Continuity", "Visible Diff on Refine",
      "Inferred From Free Text"],
     "“leaks” → leak-proof  ·  “heavy” → lightweight"),
    ("✨", "Clear Decisions", PURPLE,
     ["3 Curated Picks", "Side-by-Side Compare",
      "Match Checklist (✓ ✗ ?)"],
...
add_text(s, "🏠  Lifecycle Support  —  Value continues long after the purchase",
...
add_text(s, "Claude Opus 4.7  (AI)",
         ...
         bold=True, size=12, color=WHITE, align=PP_ALIGN.CENTER)
add_text(s, "Parse free-text preferences\nMap free-text answers\nWrite per-product copy\nParse refine intent\n(every call has a fallback)",
...
alg_steps = [
    ...
    (TEAL,     WHITE, "④ Ranked Output → 3 Curated Picks",
     "Combined = General + category bonus.  Differentiated picks:\nBest Fit · Budget Pick · Stretch (Largest Screen / Most Capacity / Best Visibility).\nFactual tradeoff labels (Cheapest, Most leakproof) only when true within the shown set."),
]
...
stack_cards = [
    ("ETL  +  Provenance-Flagged Backfill",  MID_BLUE,
     "Playwright scrapers  ·  Amazon · Target\n"
     "Anti-bot stealth · human-like delays\n"
     "Normalize: price, rating, delivery, dedup\n"
     "Backfill flagged: scraped vs estimated vs synthesised",
     "100 products  ·  every field has provenance"),
    ("Attribute Extraction (load-time)",  TEAL,
     "Echo Show 5/8/15 → screen_inches  ·  ecosystems\n"
     "has_camera  ·  mounting (wall/counter/tilt)\n"
     "capacity_oz  ·  bottle_material  ·  drinking_style\n"
     "organizer_material  ·  organizer_visibility",
     "feeds ranker, picks, and comparison view"),
    ("Claude Opus 4.7  —  3 Call Sites",  PURPLE,
     "①  Parse free-text → category + preferences\n"
     "②  Map free-text answers → structured values\n"
     "③  Write personal copy + parse refine intent\n"
    ...
    ("React Frontend  —  5 Stages",  ORANGE,
     "①  Need Discovery  —  situational context\n"
     "②  Clarification Q&A  —  6 decision-relevant chips\n"
     "③  3 Curated Picks  —  Best Fit · Budget · Stretch\n"
     "④  Why This Fits  —  rule-grounded checklist\n"
     "⑤  Lifecycle  —  realistic post-purchase support",
     "+ Comparison overlay  ·  Save-for-later  ·  Diff history"),
]
...
findings = [
    ("Capability gaps addressed",
     "7 of 7 gaps resolved — no competing platform\naddresses more than 2 simultaneously"),
    ("Information completeness",
     "Decision-critical attributes surfaced inline:\nscreen size, ecosystem, camera, capacity, material"),
    ("Interaction efficiency",
     "End-to-end decision in 5 stages, zero\nexternal page navigation required"),
    ("Decision transparency",
     "Every pick shows a ✓/✗/? checklist mapping each\nuser preference to a match status — no surprises"),
]
...
demo_steps = [
    ...
    ("②  Clarification (6 chips)",
     "Gym · Stainless · FreeSip · Insulated\nLarge capacity · Under $40"),
    ("③  3 Curated Picks",
     "Best Fit Owala 24oz FreeSip · Budget Zak 19oz\nStretch Owala 32oz [Most capacity, Most leakproof]"),
    ("④  Why + Checklist",
     "5 rule-grounded reasons + ✓/✗/? per chip.\nBudget Pick honestly shows ✗ Large capacity (19oz)."),
    ("⑤  Refine + Compare",
     "‘larger’ → flips Best Fit to 32oz.\nCompare 3 finalists side-by-side."),
]
step_h = Inches(0.85)
...
add_text(s,
         "Heuristic self-assessment against the seven capability gaps  ·  "
...
cons = [
    ...
    "✓  Honest about what's possible —\n    realistic post-purchase copy",
    "✓  Generalisable: same pattern\n    across all 3 categories",
]
fut = [
...
info_items = [
    ...
    ("Stack",       "React · FastAPI · Claude Opus 4.7 · Python · Playwright (ETL)"),
]
...
# ── Save ──────────────────────────────────────────────────────────────────────
out = "/Users/sabrina/Projects/shopping-assistant-4/docs/capstone_presentation_v4.pptx"
prs.save(out)
