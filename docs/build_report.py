...
para(
    ...
    "project proposes and implements a decision-oriented conversational shopping assistant that integrates "
    "interactive preference elicitation, multi-criteria decision modeling, constraint-aware recommendation "
    "ranking, and explainable trade-off communication into a unified end-to-end workflow. A real ETL pipeline "
    "collects product data from Amazon and Target via Playwright-based scrapers, normalizes attributes, and "
    "stores 100 products across three categories. Claude AI (Anthropic) serves as the conversational and "
    "predictive backbone, parsing user intent into structured constraints and generating natural-language "
    "explanations. A React frontend and FastAPI backend complete the full-stack implementation. Results show "
    "that the proposed system addresses structural gaps in existing tools — preference continuity, decision "
...
toc_entries = [
    ...
    ("4", "Models", "6"),
    ("  4.1", "ETL Pipeline", "6"),
    ("  4.2", "Dimensional Model", "7"),
    ("  4.3", "Predictive Models", "7"),
    ("  4.4", "Visualization", "8"),
    ("5", "Results", "9"),
    ("6", "Conclusions and Impact", "10"),
    ("7", "Contributions", "11"),
    ("8", "References", "12"),
    ("9", "Appendix — Code Listings", "13"),
]
...
para(
    ...
    "implementation using React, FastAPI, and the Claude Sonnet AI model."
)
page_break()
...
bullet("Target — primary source for water bottles and kitchen organizers, with Walmart as a fallback source.")
...
para(
    ...
    "stealth measures (human-like delays, slow scrolling, realistic browser fingerprints). Walmart was "
    "found to block scraping reliably and was used only as a fallback source."
)
...
heading(1, "Models", "4.")
para(
    "This section describes the four model components of the system: the ETL pipeline that collects and "
    "prepares product data, the dimensional model that structures it, the predictive models that drive "
    "preference parsing and recommendation ranking, and the visualization layer that presents results "
    "to users."
)

heading(2, "ETL Pipeline", "4.1")
para(
    "The ETL pipeline is implemented in Python using Playwright for browser automation. It consists of "
    "four scraper modules (scrape_amazon.py, scrape_bestbuy.py, scrape_walmart.py, scrape_target.py) "
    "and a pipeline orchestrator (run_pipeline.py) that coordinates extraction, transformation, and loading."
)
...
heading(2, "Dimensional Model", "4.2")
para(
...
heading(2, "Predictive Models", "4.3")
para(
    "Two predictive model components drive the conversational and recommendation logic: a Claude AI "
    "language model for natural language understanding and generation, and a weighted scoring model "
    "for constraint-aware product ranking."
)
para(
    "Claude Sonnet (Anthropic) serves as the primary AI backbone, invoked at four points in the "
    "decision workflow [10]:"
)
bullet("Preference parsing: Claude converts the user's conversational answers into a structured preference object (budget, priorities, constraints, trade-off tolerance).")
bullet("Preference mapping: Claude maps extracted preferences to product attributes in the database schema.")
bullet("Recommendation explanation: Claude generates natural-language justifications explaining why each product fits or partially fits the user's stated needs.")
bullet("Supplemental suggestions: Claude generates post-purchase lifecycle suggestions based on the purchased product and user context.")
para(
    "The recommendation ranking model applies weighted scoring over the feasible product set — those "
    "satisfying all hard constraints (budget ceiling, delivery deadline). Products are scored using a "
    "linear combination of normalized attribute values, with weights derived from the user's stated "
    "priorities. Tie-breaking rules favor higher rating, then lower delivery time, then lower price. "
    "This approach is intentionally transparent and interpretable, prioritizing decision clarity over "
    "prediction accuracy."
)

heading(2, "Visualization", "4.4")
para(
...
para(
    ...
    "that addresses structural gaps in existing shopping AI tools."
)
para(
...
bullet("A working full-stack implementation validated through scenario-based evaluation, showing that preference elicitation, constraint-aware ranking, and explainable trade-offs can be delivered within a single conversational interface.")
...
para(
    "This capstone project was completed individually by Chenghui Tan. All components of the system "
    "were designed, implemented, and evaluated by the author. Specific contributions include:"
)
bullet("System design and five-layer architecture: Defining the overall decision-support framework and mapping research objectives to implementable system components.")
bullet("Data engineering: Designing and implementing the Playwright-based ETL pipeline, including scraper modules for four retailers, the pipeline orchestrator with fallback logic, and the normalize.py transformation utilities.")
bullet("Predictive model development: Integrating Claude AI at four call sites within the conversational workflow, implementing the weighted scoring recommendation engine, and designing the preference elicitation question flow.")
bullet("Frontend development: Implementing the four-stage React SPA including all UI components, API client, and state management.")
bullet("Backend development: Implementing the FastAPI REST API with four endpoints, Claude client integration, and product database querying logic.")
bullet("Evaluation: Designing and executing the scenario-based walkthrough, capability comparison table, and heuristic assessment.")
bullet("Documentation: Preparing this report, the slide deck, and all project materials.")

heading(2, "AI Tool Contributions", "7.2")
para(
    "AI tools were used in the following capacities during this project:"
)
bullet("Claude (Anthropic Sonnet) — core AI backbone of the application: preference parsing, product attribute mapping, recommendation explanation generation, and lifecycle suggestion generation. Also used during development for code assistance, debugging, and documentation drafting.")
...
refs = [
    ...
    "[3] Google, 'Google Shopping Help Center,' Google LLC. [Online]. Available: https://support.google.com/merchants/. [Accessed: Apr. 2025].",
    "[4] Amazon, 'Rufus: AI Shopping Assistant,' Amazon.com, Inc. [Online]. Available: https://www.aboutamazon.com/news/retail/amazon-rufus. [Accessed: Apr. 2025].",
    "[5] OpenAI, 'ChatGPT Shopping Integration,' OpenAI. [Online]. Available: https://openai.com/chatgpt. [Accessed: Apr. 2025].",
    ...
    "[7] Perplexity AI, 'Perplexity AI: Ask Anything,' Perplexity AI, Inc. [Online]. Available: https://www.perplexity.ai. [Accessed: Apr. 2025].",
    ...
    "[10] Anthropic, 'Claude API Documentation,' Anthropic PBC. [Online]. Available: https://docs.anthropic.com. [Accessed: Apr. 2025].",
    "[11] Playwright Contributors, 'Playwright for Python,' Microsoft. [Online]. Available: https://playwright.dev/python/. [Accessed: Apr. 2025].",
    "[12] FastAPI Contributors, 'FastAPI Documentation,' Sebastián Ramírez. [Online]. Available: https://fastapi.tiangolo.com. [Accessed: Apr. 2025].",
]
for ref in refs:
    p = doc.add_paragraph()
    p.paragraph_format.left_indent  = Inches(0.3)
...
# ── Save ──────────────────────────────────────────────────────────────────────
out = "/Users/sabrina/Projects/shopping-assistant-4/docs/capstone_report.docx"
doc.save(out)
print(f"Saved → {out}")
