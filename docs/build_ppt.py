...
row_h = Inches(0.65); header_y = Inches(1.78)
...
add_rect(s, Inches(0.45), Inches(6.6), Inches(12.4), Inches(0.48), NAVY)
add_text(s,
         "→  Our system is the only one that addresses all six gaps simultaneously.  "
         "Here is how we built it.",
         Inches(0.65), Inches(6.65), Inches(12.0), Inches(0.4),
...
clusters = [
    ...
    ("👤", "Built for You", MID_BLUE,
     ["Personal AI Guide", "Preference Memory", "Decision Model"],
     "remembers  ≤ $80  ·  3-day  ·  quiet home"),
    ("✨", "Clear Results", PURPLE,
     ["Structured — Not Text Heavy", "Full Attributes", "Explainable Trade-offs"],
...
add_text(s, "Stage 1 — Onboarding\nStage 2 — Elicitation Q&A\nStage 3 — Recommendations\nStage 4 — Refinement",
...
cs_items = [
    ("🤖  Claude — Preference Parsing",    MUTED),
    ("⚙️  Rule-Based — Ranking & Filtering", ORANGE),
    ("🤖  Claude — Trade-off Explanation", MUTED),
]
...
stack_cards = [
    ...
    ("React Frontend  —  4 Stages",  ORANGE,
...
findings = [
    ("Capability gaps addressed",
     "6 of 6 gaps resolved — no competing platform\naddresses more than 2 simultaneously"),
    ("Information completeness",
     "All decision-critical attributes surfaced:\nprice, rating, delivery, promos — in one view"),
    ("Interaction efficiency",
     "End-to-end decision in 4 stages, zero\nexternal page navigation required"),
    ("Decision transparency",
     "Every recommendation includes a structured\nexplanation mapped to user priorities"),
]
...
demo_steps = [
    ...
    ("②  Elicitation",
     "5 guided questions: budget, screen,\ndelivery, household, voice control"),
    ("③  Recommendations",
     "Ranked list with price, rating, delivery\n+ trade-off explanation per product"),
    ("④  Refinement",
     "Deprioritize screen size → re-ranked\nby delivery + lifecycle suggestions"),
]
for i, (step, desc) in enumerate(demo_steps):
    y = Inches(2.5) + i * Inches(1.0)
    add_text(s, step, Inches(7.1), y,
             Inches(5.6), Inches(0.38), bold=True, size=11, color=ICE)
    add_text(s, desc, Inches(7.1), y + Inches(0.36),
             Inches(5.6), Inches(0.55), size=10, color=WHITE)
...
add_text(s,
         "Heuristic self-assessment against the six capability gaps  ·  "
...
lims = [
    ...
    "⚠  Static data — prices & delivery\n    change dynamically",
    "⚠  Rule-based ranking — weights\n    inferred, not learned",
]
# Fixed: "designed to address" not "reduces" (no user study to prove it)
cons = [
    "✓  Decision support feasible\n    end-to-end with Claude AI",
    "✓  Structured elicitation designed\n    to address cognitive load",
    "✓  Explainable trade-offs increase\n    decision transparency",
    "✓  Real ETL pipeline validates\n    data engineering approach",
    "✓  Lifecycle layer extends value\n    beyond the point of purchase",
]
fut = [
...
info_items = [
    ...
    ("Stack",       "React · FastAPI · Claude AI · Playwright · Python"),
]
...
# ── Save ──────────────────────────────────────────────────────────────────────
out = "/Users/sabrina/Projects/shopping-assistant-4/docs/capstone_presentation.pptx"
prs.save(out)
