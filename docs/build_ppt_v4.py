"""
Capstone Presentation v2 — 13 slides (renamed from v3 → v2 per overnight build)
A Decision-Oriented Conversational Shopping Assistant
Chenghui Tan — CSUEB · April 2026

v4 build adds two slides over v3:
  · Slide 5 (NEW): Five-Scene UI Walkthrough — visual tour of the demo flow
  · Slide 10 (NEW): Three Demo Scenarios — kitchen / gym / cabinet, one per category
All other slides preserved from v3 with slide-number footers updated.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 5 — Five-Scene UI Walkthrough  (LIGHT)  [v4 NEW]
# ══════════════════════════════════════════════════════════════════════════════
s = prs.slides.add_slide(BL)
bg(s, LIGHT_BG)
top_accent(s, NAVY)

section_label(s, "04  ·  Demo Flow", NAVY)
slide_title(s, "Five Scenes  ·  One Decision Journey", NAVY, 28)

scene_data = [
    ("1", "Frustration",      "User vents pain;\nassistant replies with empathy."),
    ("2", "Clarification",    "Chip questions translate\nvague needs into structured prefs."),
    ("3", "Recommendations",  "12 ranked tiles —\nattributes visible inline."),
    ("4", "Why this fits",    "Reasons + trade-off card +\ntransparency banner."),
    ("5", "Lifecycle",        "Daily use, schedule, menu,\npost-purchase value."),
]
card_y = Inches(1.85); card_w = Inches(2.45); card_h = Inches(4.5)
gap = Inches(0.13)
total_w = 5 * card_w + 4 * gap
left = (W - total_w) / 2
for i, (num, title, body) in enumerate(scene_data):
    x = left + i * (card_w + gap)
    add_rect(s, x, card_y, card_w, card_h, WHITE, line=True, line_color=ICE_DEEP)
    add_rect(s, x, card_y, card_w, Inches(0.55), NAVY)
    add_text(s, "Scene " + num, x + Inches(0.2), card_y + Inches(0.12),
             card_w - Inches(0.4), Inches(0.35),
             bold=True, size=12, color=ICE, font=HEADER_FONT)
    add_text(s, title, x + Inches(0.2), card_y + Inches(0.7),
             card_w - Inches(0.4), Inches(0.45),
             bold=True, size=15, color=NAVY, font=HEADER_FONT)
    add_text(s, body, x + Inches(0.2), card_y + Inches(1.25),
             card_w - Inches(0.4), Inches(2.4),
             size=11, color=DARK_TEXT)
    # Tag at bottom
    layer_tag = ["Layer 1", "Layer 1→2", "Layer 3", "Layer 4", "Layer 5"][i]
    add_rect(s, x + Inches(0.3), card_y + card_h - Inches(0.55),
             card_w - Inches(0.6), Inches(0.32), PANEL_BG)
    add_text(s, layer_tag, x + Inches(0.3), card_y + card_h - Inches(0.5),
             card_w - Inches(0.6), Inches(0.3),
             bold=True, size=10, color=MID_BLUE, align=PP_ALIGN.CENTER, font=HEADER_FONT)

add_text(s, "Each scene corresponds to exactly one architectural layer — UI and architecture are legible to each other.",
         Inches(1.0), Inches(6.55), Inches(11.3), Inches(0.4),
         italic=True, size=12, color=MUTED_D, align=PP_ALIGN.CENTER, font=HEADER_FONT)

slide_num(s, 5, total=TOTAL, dark=True)

# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 6 — The Trust Boundary  (DARK)  [anchor slide]
# ══════════════════════════════════════════════════════════════════════════════
s = prs.slides.add_slide(BL)
bg(s, NAVY)
add_rect(s, 0, 0, W, Inches(0.06), CORAL)

section_label(s, "05  ·  The Core Architectural Decision", ICE)
...
slide_num(s, 6, total=TOTAL, dark=False)
...
section_label(s, "06  ·  Architecture", NAVY)
...
slide_num(s, 7, total=TOTAL, dark=True)
...
section_label(s, "07  ·  Recommendation Algorithm", ICE)
...
slide_num(s, 8, total=TOTAL, dark=False)
...
section_label(s, "08  ·  Data Pipeline", NAVY)
...
slide_num(s, 9, total=TOTAL, dark=True)
...
section_label(s, "09  ·  Results & Demo", ICE)
...
slide_num(s, 10, total=TOTAL, dark=False)

# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 11 — Three Demo Scenarios  (LIGHT)  [v4 NEW]
# ══════════════════════════════════════════════════════════════════════════════
s = prs.slides.add_slide(BL)
bg(s, LIGHT_BG)
top_accent(s, NAVY)

section_label(s, "10  ·  Live Demo Plan", NAVY)
slide_title(s, "Three End-to-End Scenarios — One Per Category", NAVY, 26)

scen = [
    {
        "title":  "Kitchen Assistant",
        "user":   "Sarah · 38 · busy parent",
        "input":  "\"I cook every day but my phone screen is too small to follow recipes.\"",
        "picks":  "Cooking · Under $150 · No rush",
        "top1":   "Echo Show 8 / Echo Show 5 with Smart Bulb",
        "value":  "Hands-free recipes · meal-plan dashboard · family hub",
    },
    {
        "title":  "Gym Hydration",
        "user":   "Mike · 27 · runner",
        "input":  "\"My bottle is heavy, leaks, and I can't track my water.\"",
        "picks":  "Gym · Insulated · Lightweight",
        "top1":   "Owala FreeSip 19oz Insulated ($14.99)",
        "value":  "Hydration tracker · workout reminders · flavor ideas",
    },
    {
        "title":  "Cabinet Reset",
        "user":   "Priya · 31 · small apartment",
        "input":  "\"My drawers are chaos. Every morning I waste time hunting.\"",
        "picks":  "Cabinets · Not enough space · Stackable",
        "top1":   "Brightroom Stackable Pantry Bin Set ($9–16)",
        "value":  "Weekly tidy nudges · smart restock · calm counters",
    },
]
...
for i, sc in enumerate(scen):
    ...
    header_color = [NAVY, MID_BLUE, CORAL][i]
    add_rect(s, x, card_y, card_w, Inches(0.65), header_color)
    add_text(s, sc["title"], x + Inches(0.25), card_y + Inches(0.15),
             card_w - Inches(0.5), Inches(0.4),
             bold=True, size=16, color=WHITE, font=HEADER_FONT)
    add_text(s, sc["user"], x + Inches(0.25), card_y + Inches(0.85),
             card_w - Inches(0.5), Inches(0.3),
             italic=True, size=11, color=MUTED_D)

    add_text(s, "Opening prompt", x + Inches(0.25), card_y + Inches(1.25),
             card_w - Inches(0.5), Inches(0.25),
             bold=True, size=10, color=CORAL, font=HEADER_FONT)
    add_text(s, sc["input"], x + Inches(0.25), card_y + Inches(1.5),
             card_w - Inches(0.5), Inches(0.85),
             italic=True, size=11, color=DARK_TEXT)

    add_text(s, "Chip picks", x + Inches(0.25), card_y + Inches(2.4),
             card_w - Inches(0.5), Inches(0.25),
             bold=True, size=10, color=CORAL, font=HEADER_FONT)
    add_text(s, sc["picks"], x + Inches(0.25), card_y + Inches(2.65),
             card_w - Inches(0.5), Inches(0.4),
             size=11, color=DARK_TEXT)

    add_text(s, "Top recommendation", x + Inches(0.25), card_y + Inches(3.15),
             card_w - Inches(0.5), Inches(0.25),
             bold=True, size=10, color=CORAL, font=HEADER_FONT)
    add_text(s, sc["top1"], x + Inches(0.25), card_y + Inches(3.4),
             card_w - Inches(0.5), Inches(0.5),
             bold=True, size=11, color=NAVY, font=HEADER_FONT)

    add_text(s, "Lifecycle value", x + Inches(0.25), card_y + Inches(4.0),
             card_w - Inches(0.5), Inches(0.25),
             bold=True, size=10, color=CORAL, font=HEADER_FONT)
    add_text(s, sc["value"], x + Inches(0.25), card_y + Inches(4.25),
             card_w - Inches(0.5), Inches(0.6),
             size=11, color=DARK_TEXT)

add_text(s, "Same five-layer architecture · same UI flow · three product categories — generalisability demonstrated.",
         Inches(1.0), Inches(7.0), Inches(11.3), Inches(0.4),
         italic=True, size=12, color=MUTED_D, align=PP_ALIGN.CENTER, font=HEADER_FONT)

slide_num(s, 11, total=TOTAL, dark=True)

# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 12 — Limitations · Conclusions · Future Work  (LIGHT)
# ══════════════════════════════════════════════════════════════════════════════
s = prs.slides.add_slide(BL)
bg(s, LIGHT_BG)
top_accent(s, NAVY)
...
section_label(s, "11  ·  Honest Assessment", NAVY)
...
slide_num(s, 12, total=TOTAL, dark=True)

# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 13 — Thank You / Q&A  (DARK)
# ══════════════════════════════════════════════════════════════════════════════
s = prs.slides.add_slide(BL)
bg(s, NAVY)
...
slide_num(s, 13, total=TOTAL, dark=False)

# ── Save ──────────────────────────────────────────────────────────────────────
out = "/Users/sabrina/Projects/shopping-assistant-4/docs/capstone_presentation_v2.pptx"
prs.save(out)
